...
zbozi = ("Kabelka", "Jízdenka", "Bunda", "Vstupenka na koncert", "Hračka")
...
if pocitac == 1:
    ...
    task = '{} stála před Vánoci {} Kč a byla zlevněna o {} procent.Kolik stojí nyní?'.format(zbozi,pc,procent)
    solution = sc
elif pocitac == 2:
    pc = 0.5
    while not pc % 1 == 0:
    ...
    task = '{} byla zlevněna o {} procent a nyní stojí {} Kč. Jaká byla původní cena?'.format(zbozi,procent,sc2)
    solution = pc
elif pocitac == 3:
    x = 0.5
    while not x % 1 == 0:
    ...
    task = '{} byla zdražena o {} Kč, což je {} procent ceny. Kolik stojí nyní?'.format(zbozi,okolik,procent)
    solution = x
elif pocitac == 4:
    y = 0.5
    while not y % 1 == 0:
    ...
    task = '{} byla zdražena o {} procent a nyní stojí {}. Jaká byla původní cena?'.format(zbozi,procent,sc3)
    solution = y
